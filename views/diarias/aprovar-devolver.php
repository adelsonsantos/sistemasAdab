...
<div class="panel panel-default">
    <div class="panel-heading" style="height: 80px">
        <h3 style="text-align: center">Devolver Aprovação: <?= $model->diaria_numero; ?></h3>
    </div>
</div>
...
<div style="display: none">
    <?= $form->field($modelAprovacaoDevolver, 'diaria_id')->textInput(['value'=> 12345]); ?>

    <?= $form->field($modelAprovacaoDevolver, 'diaria_devolucao_dt')->textInput(['value'=> '2017-11-01']); ?>

    <?= $form->field($modelAprovacaoDevolver, 'diaria_devolucao_hr')->textInput(['value'=> '12:00']); ?>

    <?= $form->field($modelAprovacaoDevolver, 'diaria_devolucao_func')->textInput(['value'=> 0]); ?>

    <?= $form->field($modelAprovacaoDevolver, 'super_sd')->textInput(['value'=> 0]); ?>

    <?= $form->field($modelAprovacaoDevolver, 'diaria_st')->textInput(['value'=> 100]); ?>
</div>
...
<div class="panel panel-default" style="margin-left: 209px; margin-top: 12px">
    <div class="panel-heading">
        <div class="panel-body">
            <div class="row">
                <div class="col">
                    <?= $form->field($modelAprovacaoDevolver, 'motivo_id')->dropDownList(
            ...
            <div class="row">
                <div class="col">
                    <?= $form->field($modelAprovacaoDevolver, 'diaria_devolucao_ds')->textarea(['rows' => 3, 'cols' => 60])->label('Descrição');
                    ?>
                </div>
            </div>
        </div>
    ...
    <table class="diaria" style=" width: 100%">
        <tr class="bordaMenu">
            <th class="borda" style="text-align: center; width: 50%">
                <?= Html::a('<span class="glyphicon glyphicon-save" style="color: white; font-size: 1.2em;"> Devolver </span>', ['aprovar-devolver', 'id' => $model->diaria_id], ['class' => 'btn btn-danger', 'data' => ['method' => 'post']]); ?>
            </th>
            <th class="borda" style="text-align: center; width: 50%">
